...
const Title = ({canGoTo, onClick, page}) => (<TreeItemTitle canGoTo={canGoTo} page={page}>
...
const Tree = ({pagesYouCanGoToIDs, root}) => {
  ...
  let myPages = pageList.filter((page) => root ? page.parent === root : !page.parent)
  ...
  return <TreeRoot>
    {myPages.map((page) => {
      ...
      return <TreeItem page={page} key={page.id}>
        <Title onClick={onClick} canGoTo={canGoTo} page={page}/>
        <Tree pagesYouCanGoToIDs={pagesYouCanGoToIDs} root={page.id}/></TreeItem>
    })}
  </TreeRoot>
}
...
export default () => {
  ...
  const {pageList, currentPage, goToPageId, pagesYouCanGoTo} = wizardContext;

  if (!(pageList && currentPage)) return null;
  return <Root>
    <Tree pagesYouCanGoToIDs={pagesYouCanGoTo.map((page) => page.id)}></Tree>
  </Root>
}